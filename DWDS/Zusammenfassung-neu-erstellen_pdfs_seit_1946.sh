...
pdf_datei_zwischenablage="Zusammenfassung_Woerter_seit_1946_ruecklaeufig_Zwischenablage_$(date '+%Y%m%d').pdf"
pdf_datei_endfassung="Zusammenfassung - Wörter seit 1946 rückläufig (Korpus-Zeitungen, $(date '+%Y%m%d')).pdf"
...
info_schluesselwoerter=$( ls *DWDS-Wortverlauf\ seit\ 1946*.*pdf | sed 's@ - .*@@' | sort | uniq | tr '\n' ';' | sed --regexp-extended 's@;@; @g; s@\s+\([^()]+\)@@g; s@ +$@@' )
info_schluesselwoerter_latex_href=$( ls *DWDS-Wortverlauf\ seit\ 1946*.*pdf | sed 's@ - .*@@' | sort | uniq | sed -r 'h; s@([^()]+) \(.*$@\1@; s@(.+)@ \\href{https://dwds.de/wb/&}{@; G; s@\n@@g; s@([^ ]),([^ ])@\1, \2@g; s@$@};@' | tr '\n' ' '  )

n_schluesselwoerter=$( ls *DWDS-Wortverlauf\ seit\ 1946*.*pdf | sed 's@ - .*@@' | sort | uniq | wc -l )

info_titel="${n_schluesselwoerter} Beispielwörter langsam verschwindend seit 1946 – Korpus Zeitungen ($(date '+%Y%m%d'))"
