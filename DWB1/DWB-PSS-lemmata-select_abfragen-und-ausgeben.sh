...
abhaengigkeiten_pruefen() {
  ...
  if ! [[ -x "$(command -v pandoc)" ]]; then
    printf "${ORANGE}Befehlswerkzeug${FORMAT_FREI} pandoc ${ORANGE} zum Erstellen von Dokumenten in HTML, ODT nicht gefunden: Bitte${FORMAT_FREI} pandoc ${ORANGE}über die Programm-Verwaltung installieren.${FORMAT_FREI}\n"; stufe_abbruch=1;
  fi
...
-s,    --stillschweigend   Kaum Meldungen ausgeben
       ...
       --entwickeln,--debug Befehlsmeldungen ausgeben, die ausgeführt werden (für Programmier-Entwicklung)
...
if [[ -e "${json_speicher_datei}" ]];then
  ...
  def szErsetzen:
    ...
    elif ($w|test("^auszer"))    then ($w|sub("^auszer"; "außer"))
    elif ($w|test("^Auszer"))    then ($w|sub("^Auszer"; "Außer"))
    ...
    elif ($w|test("^genosz"))    then ($w|sub("^genosz"; "genoß"))
    elif ($w|test("^Genosz"))    then ($w|sub("^Genosz"; "Genoß"))
    ...
    elif ($w|test("^schlosz"))  then ($w|sub("^schlosz"; "schloß"))
    elif ($w|test("^Schlosz"))  then ($w|sub("^Schlosz"; "Schloß"))
    elif ($w|test("^schmeisz"))  then ($w|sub("^schmeisz"; "schmeiß"))
    elif ($w|test("^Schmeisz"))  then ($w|sub("^Schmeisz"; "Schmeiß"))
    elif ($w|test("^schweisz"))  then ($w|sub("^schweisz"; "schweiß"))
    elif ($w|test("^Schweisz"))  then ($w|sub("^Schweisz"; "Schweiß"))
    elif ($w|test("^spasz"))   then ($w|sub("^spasz"; "spaß"))
    elif ($w|test("^Spasz"))   then ($w|sub("^Spasz"; "Spaß"))
    elif ($w|test("^spiesz"))   then ($w|sub("^spiesz"; "spieß"))
    elif ($w|test("^Spiesz"))   then ($w|sub("^Spiesz"; "Spieß"))
    elif ($w|test("^strasz"))   then ($w|sub("^strasz"; "straß"))
    elif ($w|test("^Strasz"))   then ($w|sub("^Strasz"; "Straß"))
    elif ($w|test("^umrisz"))   then ($w|sub("^umrisz"; "umriß"))
    elif ($w|test("^Umrisz"))   then ($w|sub("^Umrisz"; "Umriß"))
    elif ($w|test("^urgrosz"))   then ($w|sub("^urgrosz"; "urgroß"))
    elif ($w|test("^Urgrosz"))   then ($w|sub("^Urgrosz"; "Urgroß"))
    # Mittelwörter
    ...
    elif ($w|test("maszen"))      then ($w|sub("maszen"; "maßen"))
    ...
    elif ($w|test("schliesz"))   then ($w|sub("schliesz"; "schließ"))
    elif ($w|test("Schliesz"))   then ($w|sub("Schliesz"; "Schließ"))
